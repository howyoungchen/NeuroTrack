...
- `SleepAnalyzer.targetDateForAnalysis()`：睡眠按醒来日归档，实时目标始终是当前本地日期，不再因中午前后而落到不同日期。
- `SleepAnalyzer.windowFor(targetDate)`：历史日的标准候选窗口为 `targetDate - 1` 的 18:00 到 `targetDate` 的 18:00；18:00 是回查起点，不是实时分析的醒来截止点。
- `SleepAnalyzer.windowForCurrentAnalysis()`：实时分析从前一天 18:00 回查到当前时刻；即使已过当天 18:00 也继续到“现在”，不会用未来时间补齐息屏段，也不会在中午截断晚睡记录。
- 后台每 3 小时做一次兜底分析，应用回到前台时立即追加一次刷新；同一醒来日的结果会被较新的分析更新，临时权限/数据缺失不会把已有可用记录降级为缺失。
- 自定义 raw 导出可以选择任意开始/结束时间；默认值是当前睡眠周期从窗口起点到当前时刻。
...
6. 候选段必须满足：
   - 睡眠时长 120 到 780 分钟。
   - 入睡时间不晚于 `targetDate` 08:00。
   - 与 00:00 到 10:00 的核心睡眠窗口重叠至少 120 分钟。
